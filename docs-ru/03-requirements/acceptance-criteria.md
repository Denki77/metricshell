...
> Статус: принятые нормативные критерии приёмки
...
После завершения workload и до того, как final scrape может быть засчитан, MetricShell формирует только одно финальное
...
При required count `N` допустимых responses меньше `N` не удовлетворяют условию; только `N` — удовлетворяют.
