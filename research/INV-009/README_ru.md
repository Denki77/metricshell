...
Прогон macOS/LinuxKit aarch64 прошёл 78/78 внутри-контейнерных assertions и обе внешние OOM-проверки. Все 13 860 полных
candidates были прочитаны, валидированы, атомарно установлены и подтверждены живыми consumers. Параллельные readers
active state выполнили более 150 тыс. чтений и не увидели ни одного malformed/torn или неизвестного состояния.
...
Исследование остаётся в процессе, пока Ubuntu не зафиксирует fingerprint
`41c97e834fba84c771980e2563a9817509add8fd419cde367b34cde5f2e77d07` и не будет выпущен ADR.

## Допустимые значения (предварительно)
...
- до cross-environment validation shared memory не рекомендуется как default ingestion adapter.
...
- Текущие доказательства получены только на macOS/LinuxKit aarch64. Подтверждение в Ubuntu-окружении ожидается.
...
| Ubuntu run с совпадающим fingerprint                        | ожидается; подготовлен тот же runner                              |
...
На Ubuntu сначала нужно выполнить неизменённый script и проверить fingerprint. Для углублённого transport research
...
- Prototype: `prototype/`
- Runner: `run-bench.sh`
- Текущие raw evidence: `results/20260802T162350Z/`
- Подробный анализ: [report_ru.md](report_ru.md)
- ADR: ожидает Ubuntu confirmation
